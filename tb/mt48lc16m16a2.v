...
`ifdef INIT_CLEAR_MEM_BANKS  // Added, jb
    ...
    for (mem_cnt = 0; mem_cnt < mem_sizes; mem_cnt = mem_cnt + 1) begin
      Bank0[mem_cnt] = 0;
      Bank1[mem_cnt] = 0;
      Bank2[mem_cnt] = 0;
      Bank3[mem_cnt] = 0;
...
`endif
  ...
  always @(posedge Sys_clk) begin
    ...
    if (Mode_reg_enable === 1'b1) begin
      ...
      if (Pc_b0 === 1'b0 && Pc_b1 === 1'b0 && Pc_b2 === 1'b0 && Pc_b3 === 1'b0) begin
        $display("%m : at time %t ERROR: all banks must be Precharge before Load Mode Register",
                 $time);
      end
    ...
    if (Write_enable == 1'b1) begin
      ...
      if (Data_in_enable == 1'b1) begin
        ...
        if (Auto_precharge[RW_interrupt_bank] == 1'b1 && Write_precharge[RW_interrupt_bank] == 1'b1) begin
          ...
          if (Debug) begin
            $display(
                "%m : at time %t NOTE : Read Bank %h interrupt Write Bank %h with Autoprecharge",
                $time, Ba, RW_interrupt_bank);
          end
        end
      end
      ...
      if (Data_out_enable == 1'b1) begin
        ...
        if (Auto_precharge[RW_interrupt_bank] == 1'b1 && Read_precharge[RW_interrupt_bank] == 1'b1) begin
          ...
          if (Debug) begin
            $display(
                "%m : at time %t NOTE : Write Bank %h interrupt Read Bank %h with Autoprecharge",
                $time, Ba, RW_interrupt_bank);
          end
        end
      end
    ...
    if (Data_in_enable == 1'b1) begin  // Writing Data to Memory
      ...
      if (Dqm !== 2'b11) begin
        ...
        if (Debug) begin
          $display("%m : at time %t WRITE: Bank = %h Row = %h, Col = %h, Data = %h", $time, Bank,
                   Row, Col, Dq_dqm);
        end
      end else begin
        if (Debug) begin
          $display("%m : at time %t WRITE: Bank = %h Row = %h, Col = %h, Data = Hi-Z due to DQM",
                   $time, Bank, Row, Col);
        end
      end
      ...
      if (Dqm_reg0 !== 2'b11) begin
        Dq_reg = #tAC Dq_dqm;
        if (Debug) begin
          $display("%m : at time %t READ : Bank = %h Row = %h, Col = %h, Data = %h", $time, Bank,
                   Row, Col, Dq_reg);
        end
      end else begin
        Dq_reg = #tHZ{data_bits{1'bz}};
        if (Debug) begin
          $display("%m : at time %t READ : Bank = %h Row = %h, Col = %h, Data = Hi-Z due to DQM",
                   $time, Bank, Row, Col);
        end
      end
  ...
  specify
    specparam tAH = 0.8,  // Addr, Ba Hold Time
    tAS
    =
    1.5
    ,  // Addr, Ba Setup Time
